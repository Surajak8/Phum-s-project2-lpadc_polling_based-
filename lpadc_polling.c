...
PRINTF("Start breathing detection...\r\n");
...
while (1)
{
    LPADC_DoSoftwareTrigger(DEMO_LPADC_BASE, 1U); // Trigger ADC

    // ✅ Fixed: Added the FIFO index (0U)
    while (!LPADC_GetConvResult(DEMO_LPADC_BASE, &mLpadcResultConfigStruct, 0U)) {}

    adc_value = mLpadcResultConfigStruct.convValue >> g_LpadcResultShift;
   
    PRINTF("ADC: %d\r\n", adc_value);

    if (adc_value > 2500 && !breath_detected)
    {
        breath_count++;
        breath_detected = true;
        PRINTF("Breath #%d detected\r\n", breath_count);
    }
    ...
    SDK_DelayAtLeastUs(200000, SystemCoreClock); // ~200ms
}
